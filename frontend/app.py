...
if "show_resolution_buttons" not in st.session_state:
    st.session_state.show_resolution_buttons = False
...
if prompt := st.chat_input("증상이나 궁금한 점을 입력해주세요 (예: 스마트폰 발열이 너무 심해요)"):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("답변을 분석하고 있습니다..."):
            ...
            response, show_buttons = get_chat_response(
                question=prompt, 
                selected_device=selected_device, 
                thread_id=st.session_state.thread_id
            )
            st.session_state.show_resolution_buttons = show_buttons
...
# 해결 여부 버튼
if st.session_state.get("show_resolution_buttons"):
    col1, col2 = st.columns(2)
    with col1:
        if st.button("✅ 해결됐어요!", use_container_width=True):
            st.session_state.show_resolution_buttons = False
            st.session_state.messages.append({"role": "user", "content": "해결됐어요"})
            ...
            st.rerun()
    with col2:
        if st.button("❌ 그래도 안돼요", use_container_width=True):
            st.session_state.show_resolution_buttons = False
            st.session_state.messages.append({"role": "user", "content": "서비스센터 찾아줘"})
            response, _ = get_chat_response(
                question="서비스센터 찾아줘",
                selected_device=selected_device,
                thread_id=st.session_state.thread_id
