...
서비스센터 방문을 원하거나 자가수리가 어렵다고 판단될 경우  
카카오맵 API를 통해 사용자 위치 기반 가까운 삼성전자 서비스센터를 실시간으로 안내하는 완성형 CS 솔루션입니다.
...
| 구분 | 기술 | 설명 |
| --- | --- | --- |
| 언어 | Python 3.12 | 전체 파이프라인 구현 핵심 언어 |
| LLM | GPT-4 Turbo | 답변 생성, 의도 분류, 쿼리 변환 |
| RAG Framework | LangChain / LangGraph | 하이브리드 검색 + 조건부 라우팅 RAG 파이프라인 |
| 임베딩 | OpenAI text-embedding-3-small | 텍스트 벡터 임베딩 |
| 웹 프레임워크 | FastAPI + Uvicorn | 고성능 비동기 API 서버 |
...
| 데이터 처리 | Pandas / OpenPyXL | FAQ 데이터 전처리 및 Excel 파싱 |
| PDF 파싱 | PyPDF | 자가수리 매뉴얼 PDF 파싱 |
